...
`prod` and `dev` both sit at the end of Phase 4. Nothing has been installed in a shop yet, so
`prod` means "last known-good, ready to install" rather than "running somewhere" — it will start
meaning the latter the day the first till is set up.

Historical phase branches (`feature/phase-0-scaffold` … `feature/phase-4-auth`) point at the commit
