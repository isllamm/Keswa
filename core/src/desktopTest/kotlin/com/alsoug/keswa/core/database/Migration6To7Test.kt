...
/**
 * Wholesale arrives against a database full of retail trading.
 *
 * Two of the changes touch existing tables — `sale.customerId` and `sale_line.packId` — so this is
 * the test that proves a shop's retail history is untouched by the arrival of customers who buy
 * on account.
 */
class Migration6To7Test {

    private val directory: File = Files.createTempDirectory("keswa-migration-6-7").toFile()
    ...
    private fun seedVersion6() {
        SchemaFixture.createDatabaseAtVersion(databaseFile, SchemaFixture.exportedSchema(6))
    ...
    @Test
    fun `a year of retail trading survives the arrival of wholesale`() = runBlocking {
        seedVersion6()
    ...
    fun `the new tables open empty and usable`() = runBlocking {
        ...
        assertEquals(emptyList(), database.customerDao().getAll())
        assertEquals(emptyList(), database.assortmentPackDao().getAll())
        assertEquals(emptyList(), database.customerLedgerDao().customersOwing())
        database.close()
    }

    @Test
    fun `existing retail sales simply have no customer`() = runBlocking {
        seedVersion6()

        val database = open()

        // Null rather than a placeholder: those sales were walk-ins, and saying so is honest.
        assertNull(database.saleDao().getById("sale-1")?.customerId)
        assertNull(database.saleDao().getLines("sale-1").single().packId)
        database.close()
    }
}
