...
/**
 * The first migration that alters an existing table rather than only adding new ones.
 *
 * `ALTER TABLE … ADD COLUMN` with no default is the one alteration SQLite does cheaply: existing
 * rows get `NULL`, nothing is rewritten, and a shop with a year of trading pays nothing for the
 * upgrade. This test exists to prove that the year of trading is still there afterwards.
 */
class Migration4To5Test {

    private val directory: File = Files.createTempDirectory("keswa-migration-4-5").toFile()
    ...
    private fun seedVersion4() {
        SchemaFixture.createDatabaseAtVersion(databaseFile, SchemaFixture.exportedSchema(4))
    ...
    @Test
    fun `a year of trading survives the arrival of receiving`() = runBlocking {
        seedVersion4()
    ...
    @Test
    fun `movements written before this phase admit they have no cost basis`() = runBlocking {
        seedVersion4()

        val database = open()

        val movement = database.stockLedgerDao().getMovements("var-1", "loc-1").single()
        // Null rather than zero: those movements genuinely had no cost recorded, and a zero would
        // read as "it was free".
        assertNull(movement.unitCostPiastres)
    ...
    fun `the new tables open empty and usable`() = runBlocking {
        ...
        assertEquals(emptyList(), database.stockReceiptDao().recent("loc-1", 10))
        assertNull(database.stockCountDao().getOpen("loc-1"))
        database.close()
    }
}
